...
export default class Grid extends React.Component {
	...
	renderGridItem = info => {
		// index of the item, which is passed to us automatically by the FlatList.
		const { index } = info;
		const { renderItem, numColumns, itemMargin } = this.props;
		//  width can change sizes depending on device orientation,
		//  multitasking mode, etc, so it’s best to do this within the render path.
		const { width } = Dimensions.get('window');
		// calculate the dimensions of each item we’ll render
